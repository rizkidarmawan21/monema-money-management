...
const update = async () => {
    ...
    axios.put(route('akun.update', { 'id': props.data.id }), form.value)
        .then((res) => {
            emit('close')
            emit('successSubmit')
            form.value = ref({})
...
const create = async () => {
    isLoading.value = true
    axios.post(route('akun.create'), form.value)
        .then((res) => {
            emit('close')
            emit('successSubmit')
            form.value = ref({})
        ...
        <template v-slot:content>
            <div class="grid grid-cols-2 gap-3">
                <div class="col-span-2">
                    <VInput placeholder="Masukkan Nama Akun" label="Nama Akun" :required="true" v-model="form.nama_akun"
                        :errorMessage="formError.nama_akun" @update:modelValue="formError.nama_akun = ''" />
                </div>
                <div class="col-span-2">
