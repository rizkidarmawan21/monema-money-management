...
const heads = ["No", "Nama", "Akun Saldo", "Nominal", "Tanggal", ""]
...
const getData = debounce(async (page) => {
    axios.get(route('transaksi.masuk.get-data'), {
        params: {
            page: page,
...
const deleteHandle = async () => {
    axios.delete(route('transaksi.masuk.delete', { 'id': itemSelected.value.id })
    ).then((res) => {
        notify({
            type: "success",
            group: "top",
...
<template>
    ...
    <div class="flex items-center justify-between mb-4 sm:mb-6">
        <h1 class="text-2xl font-bold md:text-3xl text-slate-800">Data Pemasukan</h1>
    </div>
    <div class="bg-white border rounded-sm shadow-lg border-slate-200" :class="isLoading && 'min-h-[40vh] sm:min-h-[50vh]'">
        <header class="items-center justify-between block px-4 py-6 sm:flex">
            <h2 class="font-semibold text-slate-800">
                Semua Pemasukan <span class="text-slate-400 !font-medium ml">({{ pagination.total }})</span>
            </h2>
