...
const update = async () => {
    ...
    axios.put(route('transaksi.masuk.update', { 'id': props.data.id }), form.value)
        .then((res) => {
            emit('close')
            emit('successSubmit')
            form.value = ref({})
...
const create = async () => {
    isLoading.value = true
    axios.post(route('transaksi.masuk.create'), form.value)
        .then((res) => {
            emit('close')
            emit('successSubmit')
            form.value = ref({})
        ...
        <template v-slot:content>
            <div class="grid grid-cols-2 gap-3">
                <div class="col-span-2 sm:col-span-1">
                    <VInput placeholder="Masukkan Nama Pemasukan" label="Nama Pemasukan" :required="true"
                        v-model="form.nama_pemasukkan" :errorMessage="formError.nama_pemasukkan"
                        @update:modelValue="formError.nama_pemasukkan = ''" />
                </div>
                <div class="col-span-2 sm:col-span-1">
