...
const heads = ["No", "Nama", "Telepon", "Jabatan", "Tempat Kerja", "Alamat", "Keterangan", ""]
...
const getData = debounce(async (page) => {
    axios.get(route('karyawan.get-data'), {
        params: {
            page: page,
...
const deleteHandle = async () => {
    axios.delete(route('karyawan.delete', { 'id': itemSelected.value.id })
    ).then((res) => {
        notify({
            type: "success",
            group: "top",
...
<template>
    ...
    <div class="flex items-center justify-between mb-4 sm:mb-6">
        <h1 class="text-2xl font-bold md:text-3xl text-slate-800">Data Karyawan</h1>
    </div>
    <div class="bg-white border rounded-sm shadow-lg border-slate-200" :class="isLoading && 'min-h-[40vh] sm:min-h-[50vh]'">
        <header class="items-center justify-between block px-4 py-6 sm:flex">
            <h2 class="font-semibold text-slate-800">
                Semua Karyawan <span class="text-slate-400 !font-medium ml">({{ pagination.total }})</span>
            </h2>
        ...
        <VDataTable :heads="heads" :isLoading="isLoading">
            ...
            <tr v-for="(data, index) in query" :key="index" v-else>
                ...
                <td class="h-16 px-4"> {{ data.jabatan ?? '-' }} </td>
                <td class="h-16 px-4"> {{ data.tempat_kerja ?? '-' }} </td>
                <td class="h-16 px-4"> {{ data.alamat ?? '-' }} </td>
